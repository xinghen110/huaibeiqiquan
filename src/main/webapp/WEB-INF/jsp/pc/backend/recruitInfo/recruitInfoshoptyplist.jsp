...
<ry:authorize ifAllGranted="${authMap.SYSTEM_AUTH}">
	<li><a class="add" onclick="add('shoptype/toEdit?homeposition=${homeposition}&parentNum=${parentNum}','添加招聘分类',900,550,'main_')" ><span>添加</span></a></li>
	...
	<tbody>	
		<c:forEach var="item" items="${pageList.result}" varStatus="row">
			...
			<!-- <td><input name="ids" value="${item.shopTypeId }" type="checkbox"></td>-->
				...
				<ry:authorize ifAllGranted="${authMap.SYSTEM_AUTH}">
				<td>
				<a title="修改招聘分类"   onclick="add('shoptype/toEdit?shopTypeNum=${item.shopTypeNum}&parentNum=${parentNum}','修改招聘分类',900,550,'main_')"  rel="users_saveedit" class="btnEdit">修改</a>				
				<a title="您确定删除招聘分类信息吗？" target="ajaxTodo" href="shoptype/del?shopTypeNum=${item.shopTypeNum}" class="btnDel">删除</a>
				</td>
				</ry:authorize>
				</tr>
		</c:forEach>
