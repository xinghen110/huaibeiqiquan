...
(function($){
	...
	var Datepicker = function(sDate, opts) {
		this.opts = $.extend({
			...
			minDate:"1900-01-01",
			maxDate:"2099-12-31",
			minId:"minId",//值为id，用于两个时间的范围  minId开始时间的id（直接在input里指定使用minId="xx"）
			maxId:"maxId",//值为id，用于两个时间的范围  maxId结束时间的id（直接在input里指定使用maxId="xx"）
			mmStep:1,
			ssStep:1
		}, opts);
		...
		/**
		 * 判断时间区间 的最小值 是否有值
		 * 不为空就minId指定的时间控件所选择的值
		 *  做为当前时间控件的minDate
		 *  
		 *  为空就为datepicker所制定的minDate
		 */
		if($("#"+this.opts.minId).val()){
			this.opts.minDate = now.formatDateTm($("#"+this.opts.minId).val());	
		}else{
		...
		/**
		 * 判断时间区间 的最大值 是否有值
		 * 不为空就maxId指定的时间控件所选择的值
		 *  做为当前时间控件的maxDate
		 *  
		 *  为空就为datepicker所制定的maxDate
		 */
		if($("#"+this.opts.maxId).val()){
			this.opts.maxDate = now.formatDateTm($("#"+this.opts.maxId).val());	
		}else{
