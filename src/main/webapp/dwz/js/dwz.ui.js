...
function initUI(_box){
	...
	if ($.fn.datepicker){
		$('input.date', $p).each(function(){
			...
			/**
			 * maxId   minId  绑定到datepicker里  用于绑定取 时间范围区间
			 */
	...
	//整合使用ueditor编辑器
	$("textarea.ueditor", $p).each(function(){
			var $this = $(this);
			var id = $this.attr("id");
			//textarea添加 宽和高 ，并设置 背景色透明
			$('#'+id).attr("style","width:800px;height:300px;filter:alpha(opacity=0)");
			//UE.getEditor(id);
			//每次新进一页面 -如果有 ueditor编辑器就重新实例化一个ueditor编辑器
			var ueditor=new baidu.editor.ui.Editor();
			 //禁止随着编辑器内容的增加 ，高度增加
			ueditor.autoHeightEnabled = false;
			ueditor.render(id);
