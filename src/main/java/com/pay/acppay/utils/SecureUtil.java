...
public class SecureUtil {
    private static Logger log = LoggerFactory.getLogger(SecureUtil.class);
    /**
     * 算法常量： SHA1
     */
    ...
    public static String sm3X16Str(String data, String encoding) {
        ...
        for (byte aByte : bytes) {
            if (Integer.toHexString(0xFF & aByte).length() == 1) {
                ...
                sm3StrBuff.append(Integer.toHexString(0xFF & aByte));
            }
        }
    ...
    public static byte[] signBySoft(PrivateKey privateKey, byte[] data)
            throws Exception {
        return signBySoft256(privateKey, data, BC_PROV_ALGORITHM_SHA1RSA);
    }

    private static byte[] signBySoft256(PrivateKey privateKey, byte[] data, String alg)
            throws Exception {
        byte[] result;
        Signature st = Signature.getInstance(alg, "BC");
    ...
    public static byte[] signBySoft256(PrivateKey privateKey, byte[] data)
            throws Exception {
        return signBySoft256(privateKey, data, BC_PROV_ALGORITHM_SHA256RSA);
    }
    ...
    private static byte[] pin2PinBlock(String aPin) {
        ...
        try {
            ...
            log.error("pin2PinBlock error:{}", e.getMessage());
        }
