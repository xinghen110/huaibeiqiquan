...
import com.pay.acppay.utils.SDKConfig;
...
public class PayChannels {
    ...
    public String yinshengPay(TOrderInfo orderInfo, String bank, Model model, TUser currentUser){
        ...
        model.addAttribute("time", getTime(orderInfo));
    ...
    //银联支付
    public String acpPay(TOrderInfo orderInfo, String bank, Model model, TUser currentUser){
        logger.warn(">>>>> 【银联支付】订单支付 请求信息.");
        ...
        requestData.put("orderId", orderInfo.getOrderNum());             //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
        requestData.put("txnTime", getTime(orderInfo));             //订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
        requestData.put("currencyCode", "156");         //交易币种(境内商户一般是156 人民币)
        requestData.put("txnAmt", orderInfo.getActualPrice().multiply(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).toString());  //交易金额，单位分，不要带小数点
        ...
        Map<String, String> reqData = AcpService.sign(requestData,"UTF-8");  //报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。

        String s = SDKUtil.coverMap2String(reqData);
        logger.warn("银联支付参数："+s);
        for (Map.Entry<String, String> next : reqData.entrySet()) {
            model.addAttribute(next.getKey(), next.getValue());
        }
        //获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acpsdk.frontTransUrl
        model.addAttribute("requestUrl", config.getFrontRequestUrl());
        return "pay/acpPay";
    }

    private String getTime(TOrderInfo orderInfo){
        return DateUtils.doFormatDate(orderInfo.getOrderCreateTime(), "yyyyMMddHHmmss");
    }
}
